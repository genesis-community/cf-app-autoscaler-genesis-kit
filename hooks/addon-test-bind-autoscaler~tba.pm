...
package Genesis::Hook::Addon::AppAutoscaler::TestBindAutoscaler v4.0.0;
...
sub cmd_details {
  return
  "Tests binding the Autoscaler service broker to your deployed CF and then removes it.\n";
}
...
sub perform {
  ...
  run("cf create-service-broker test-bind-autoscaler \"$username\" \"$password\" \"$url\"");
  run("cf enable-service-access autoscaler -b test-bind-autoscaler");

  info("\n#G{[OK]} Successfully created test-bind-autoscaler service broker.");

  # TODO: Check the return values of run command to handle error cases.
  run("cf delete-service-broker test-bind-autoscaler -f");

  info("\n#G{[OK]} Successfully deleted test-bind-autoscaler service broker.");
